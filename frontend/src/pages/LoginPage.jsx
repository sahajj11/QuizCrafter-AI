    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (res.data.token) {
                ...
                // 2. Extract and Save USERNAME from the response
                //    This is the data your LightDashboardLayout is looking for!
                if (res.data.user && res.data.user.username) {
                    localStorage.setItem('username', res.data.user.username);
                } else {
                    // Fallback in case backend doesn't provide username directly
                    // This is optional but good practice:
                    localStorage.setItem('username', 'QuizCrafter User'); 
                }
                
                // 3. Save User Role (if applicable, using a static value here)
                localStorage.setItem('userRole', 'Pro Member'); 
